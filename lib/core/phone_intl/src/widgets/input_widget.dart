...
import 'package:help4kids_front/core/phone_intl/src/providers/country_provider.dart';
import 'package:help4kids_front/core/phone_intl/src/utils/formatter/as_you_type_formatter.dart';
import 'package:help4kids_front/core/phone_intl/src/utils/phone_number.dart';
...
import 'package:help4kids_front/core/phone_intl/src/utils/widget_view.dart';
import 'package:help4kids_front/core/phone_intl/src/widgets/selector_button.dart';
...
enum PhoneInputSelectorType { dropdown, bottomSheet, dialog }
...
class InternationalPhoneNumberInput extends StatefulWidget {
  ...
  const InternationalPhoneNumberInput(
      ...
      this.cursorColor,
      this.autofillHints,
      this.countries, required this.onCountryChanged});
...
class _InputWidgetState extends State<InternationalPhoneNumberInput> {
  ...
  void initialiseWidget() async {
    if (widget.initialValue != null) {
      ...
      if (widget.initialValue!.phoneNumber != null &&
        ...
        String phoneNumber = widget.initialValue!.parseNumber();

        var phone = await PhoneNumberUtil.formatAsYouType(
                phoneNumber:
                    "${country?.dialCode ?? ''}${phoneNumber.replaceAll(RegExp(r'[^\d+]'), '')}",
                isoCode: country?.alpha2Code ?? '') ??
            '';
        var phoneWithoutDial =
            phone.replaceAll(country?.dialCode ?? '', '').trimLeft();
        controller!.text = phoneWithoutDial;
        controller!.selection = TextSelection.fromPosition(
            TextPosition(offset: phoneWithoutDial.length));
      }
  ...
  void phoneNumberControllerListener() {
    if (mounted) {
      ...
      PhoneNumberUtil.formatAsYouType(
        ...
        Flexible(
          child: TextFormField(
            ...
            inputFormatters: [
              ...
              widget.formatInput
                  ? AsYouTypeFormatter(
                      isoCode: state.country?.alpha3Code ?? '',
                      dialCode: state.country?.dialCode ?? '',
                      onInputFormatted: (controller) {
                        state.controller?.text = controller.text;
